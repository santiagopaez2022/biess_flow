...
import ec.gov.iess.creditos.pq.servicio.SimulacionValorExigibleMontoSacServicioLocal;
import ec.gov.iess.creditos.pq.servicio.SimulacionValorExigibleMontoSacServicioRemoto;
import ec.gov.iess.creditos.pq.util.ConstantesSAC;
import ec.gov.iess.creditos.pq.util.FuncionesUtilesSac;

@Stateless
public class SimulacionValorExigibleMontoSacServicioImpl
		implements SimulacionValorExigibleMontoSacServicioLocal, SimulacionValorExigibleMontoSacServicioRemoto {

	private static final LoggerBiess LOG = LoggerBiess.getLogger(SimulacionValorExigibleMontoSacServicioImpl.class);
	...
	private String urlSimularValorExigibleValor;
	private static final String NOMBRE_PARAMETRO_URL = "URL_SIMULARVALOREXIGLEVALOR_SAC";

	@PostConstruct
	public void init() {
		try {
			this.urlSimularValorExigibleValor = this.parametroBiessDao
	...
	private OperacionSacResponse obtieneSimulacionValorExigible(OperacionSacRequest request)
		...
		try {
			String respuestaJson = peticionRestSacServicio.obtenerRespuestaSac(request, this.urlSimularValorExigibleValor);
			...
			if (!ConstantesSAC.RESPUESTA_EXITOSA.equals(respuesta.getCodigoRespuesta())) {
				grabarLogPeticion(urlSimularValorExigibleValor, respuesta.getCodigoRespuesta(),
	...
	@Override
	public List<DatoDeudaDto> obtenerDividendos(OperacionSacRequest operacion) throws SimulacionValorExigibleException {
		OperacionSacResponse response = obtieneSimulacionValorExigible(operacion);
		return response.getOperacionResponse().getDatosDeuda();
	}

	@Override
	public CreditoExigibleDto obtenerSimulacionExigibleValor(OperacionSacRequest operacion)
			throws SimulacionValorExigibleException {
		OperacionSacResponse response = obtieneSimulacionValorExigible(operacion);
