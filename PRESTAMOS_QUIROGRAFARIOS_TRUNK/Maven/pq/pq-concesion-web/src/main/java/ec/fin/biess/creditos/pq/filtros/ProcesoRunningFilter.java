 ...
 *
 */
public class ProcesoRunningFilter implements Filter {
	...
	private boolean blockPages(RolesBean rolesBean) {
		...
		/* Verifica si el proceso de planillas esta corriendo */
		if (rolesBean.getDatos().isProcesoPlanillasRunning()) {
			return true;
		}
		/* Verifica si el ajuste de planillas esta corriendo */
		if (rolesBean.getDatos().isReajusteTasaRunning()) {
			return true;
		}
